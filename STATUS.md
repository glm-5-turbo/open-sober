...
## SH28 (Sep 12, 2026): captured the real engine GLES dispatch-table content live. --renderframe-seedgles now dumps all 16 raw slots (BSS 0x106d3b2f0+8*N) before seeding and dladdr-resolves each. The engine's REAL renderer dispatch is a modern GLES3 pipeline (slots 4-8 = UBO/buffer funcs, 9/10 = instanced draws, 13-15 = program binary), NOT the simple clear/draw map. Slots 0-2 read as our bridge slots, confirming SH3's eglGetProcAddress interception reaches the engine's own table. Diagnostic-only (harness seeding unchanged, renders correctly). Doc docs/frontier-sh28-slotmap.md. Workspace 474/0; baselines unchanged.
