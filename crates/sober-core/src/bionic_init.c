...
/* Open-Sober harden: a never-NULL symbol resolver + a no-op fallback.
   If dlsym(RTLD_NEXT, name) returns NULL on this glibc, substitute a real
   no-op so the guest trampoline never branches to address 0 (SIGSEGV). */
__attribute__((used,noinline,visibility("default"))) long __bf_noop(void) { return 0; }
static void *__bf_dl(const char *n) {
    void *p = dlsym(RTLD_NEXT, n);
    return (p != 0) ? p : (void *)&__bf_noop;
}
